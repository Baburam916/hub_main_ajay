...
import { BookingDirectParty, BookingPartySummary, BookingTracking, Booking_summary_country_report, Booking_summary_courier_report, Booking_summary_hubpud_report, Booking_summary_report, Booking_summary_salesperson_report } from "../../../AllServices/services";
...
const newdata=salesperson?.find((item?:any)=>item?.id==data?.sales_person)
return newdata
    }
  }
  const calculateTotalInrAmount=(data?:any)=> {
   
 const sum= data.reduce((total?: any, item?: any) => total + item.inr_amount, 0);
...
const newdata = data?.map((item: any) => ({
  ...
  franchisee_code: findfranchisename(item.pickup_franchisee_id)?.ba_code || "",
  ...
  fsc_on_buy: findbuyrelateddata("fscbuy", item?.buying_data)?.inr_amount || 0,
  fsc_on_sell:
  ...
  emergency_charges_sell:
    findbuyrelateddata("emergencysell", item?.selling_data)?.inr_amount || 0,
  ...
  profit: Number(profit(item?.buying_data, item?.selling_data)).toFixed(3) || 0,
  ...
  Country_name: findcoutry(item?.delivery_country_id)?.country_name || "N.A",
  ...
  sales_person: getsalesperson(item?.pickup_franchisee_id)?.sales_person || "",
  ...
  return (
    <div className="m-auto  rounded p-4 mt-2">
      ...
      <div className="w-full">
        ...
        {maindata?.length > 0 ? <div className="w-full max-w-6xl mx-auto mt-4 px-6 py-3 bg-white rounded-lg shadow-lg">
           <>
          {bookingData ? 
        <div  style={{width: "40px", height: "40px", margin:"auto", display: "block"}}>
          <LoadingIcon icon="puff"/>
        </div>
           : <ReportCommonTable
            columns ={maindata[0]}
