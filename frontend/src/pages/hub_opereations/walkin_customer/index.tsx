  ...
  const row: any = walkindata?.map((item: any) => {
    const kycdata =
      item?.document_id_1 && item?.document_id_2 ? (
        <>
          <Link
            to={`${item?.document_path_1}?${Math.random()}`}
            target="_blank"
          >
            <Button className="bg-green-500 border-none py-1 px-2 text-white ml-2">
              {
                kycDoc
                  ?.find(
                    (elem: any) =>
                      elem?.organisation_id == item?.orgnization_id,
                  )
          ...
          <Link
            to={`${item?.document_path_2}?${Math.random()}`}
            target="_blank"
          >
    ...
    const shipperData = (
      <>
        {item?.shipper_invoice == "N/A" || item?.shipper_invoice == null ? (
          ...
          <Link
            to={`${item?.shipper_invoice}?${Math.random()}`}
            target="_blank"
          >
            ...
            <div className="flex-wrap lg:flex-nowrap flex gap-2 items-center justify-between w-full">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-[16px] font-medium">Walkin Customer</h4>
                </div>
              </div>

              <div className="flex items-center">
                <div className="flex-wrap lg:flex-nowrap flex gap-2 items-center lg:justify-end w-full">
                  <div className="flex justify-end gap-2">
                    <Button
                      className="bg-green-400 text-white  flex items-center justify-center  py-2 px-2  rounded-md  border-none"
                      onClick={() =>
                        downloadAttachment(
                    ...
                    >
                      <Lucide
                        icon="Contact"
                        className="w-[16px] h-[16px] mr-1 stroke-2.5 text-white"
                      />
                      KYC Form
                    </Button>
                    <Button
                      className="bg-mustard text-white  flex items-center justify-center  py-2 px-2 rounded-md border-none"
                      onClick={() =>
                        downloadAttachment(
                  ...
                  <div className="flex gap-2 items-center">
                    <FormInput
                      className="pr-8 pt-1 pb-1 rounded-md h-[38px]"
                      type="text"
                      value={airwaybill}
                      onChange={(e) => {
                    ...
                    <Button
                      ...
                      className="bg-gray-500 rounded-md py-1 px-3 text-white border-none h-[37px]"
                    >
                      Search
                    </Button>
                  </div>
