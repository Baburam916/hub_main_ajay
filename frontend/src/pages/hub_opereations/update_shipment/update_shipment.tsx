...
const update_shipment = ({ awbData, setShowEditBooking, setAwbNo }) => {
  ...
  const checkPga = async (booking_charges: any) => {
    try {
      setAclSpinner(true);
  ...
  const [receiverDetails, setReceiverDetails] = useState({
    ...
    consignee_state: isShipnstock
      ? ""
      : data?.state || data?.destination_country_code,
  ...
  const handleStepTwo = async () => {
    ...
    if (rateData?.destination_pincode == "" || rateData?.city == "") {
      showAlert("Please provide valid destination pincode or city", "warning");
      return;
    }
  ...
  const handleEdit = () => {
    if (
      ...
      if (Number(editDimensionData?.value) <= 0) {
        showAlert("Value should not be less than 0", "warning");
        return;
      }
                ...
                <div className="flex items-center px-4 py-3 h-full box  w-full">
                  <div className=" mr-auto  w-[100%]">
                    <div className="grid grid-cols-2 w-[100%]">
                      ...
                      <div className="px-2 py-1 w-fit text-sm text-center font-medium text-white rounded-full cursor-pointer bg-green-500 whitespace-nowrap overflow-hidden overflow-ellipsis ">
                        {Number(
                          Number(
                            franchiseeData?.available_credit_limit,
                          ).toFixed(2),
                        ).toLocaleString("en-IN")}{" "}
                        /-
                      </div>
                    ...
                    {vendorData?.length > 0 &&
                      awbData?.pickup_data?.courier_id && (
                        <div className="text-slate-500 text-sm mt-0.5">{`Booking Product: ${vendorData
                          ?.find(
                            (ele) =>
                              ele?.courier_id ==
                              awbData?.pickup_data?.courier_id,
                          )
                          ?.product_name?.toUpperCase() || "N.A."
                          }`}</div>
                      )}
                    {vendorData?.length > 0 && data?.courier_name && (
                      <div className="text-slate-500 text-sm mt-0.5">{`Current Product : ${data?.courier_name?.toUpperCase()}`}</div>
                    )}
                  </div>
                </div>
              </div>
          ...
          <div className="w-full md:w-[63%]">
            ...
            <div className="w-full md:hidden block mt-4">
              {currentFaq >= 2 ? (
                ...
                ) : allVendorData && allVendorData?.length > 0 ? (
                  <>
                        ...
                        <div className="flex justify-end gap-4">
                          ...
                          <Button
                            ...
                            disabled={
                              ...
                              (data?.courier_code?.includes("aramex") &&
                                !data?.booking_invoice_number) ||
                              ((data?.courier_code?.includes("aramex") ||
                                data?.courier_code?.includes("skynet")) &&
                                data?.consignee_gst_number == "N/A") ||
                              ((data?.courier_code?.includes("aramex") ||
                                data?.courier_code?.includes("skynet")) &&
