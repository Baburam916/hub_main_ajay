...
const change_vendor = ({ awbData, setAwbNo, setShowChangeVendor }) => {
  ...
  const [oldCounter, setOldCounter] = useState(
    awbData?.additional_data?.counter || 0,
  );
  ...
  const [data, setData] = useState({
    ...
    courier_id:
      Number(awbData?.additional_data?.counter) < 4
        ? awbData?.pickup_data?.courier_id
        : "",
    courier_code: "",
    courier_vendor_code: "",
    ...
    ...(awbData?.pickup_data?.is_domestic == 2
      ...
      : {}),
    ...(awbData?.pickup_data?.shipper_invoice ? { shipper_invoice: awbData?.pickup_data?.shipper_invoice } : {}),
  });
  ...
  useEffect(() => {
    if (isShipnstock && senderShipnstockStates?.length) {
      const matchedState = senderShipnstockStates.find(
        (state: any) =>
          state?.name?.toLowerCase() ==
          data?.consigner_state?.trim()?.toLowerCase(),
      );
      if (matchedState) {
  ...
  }, [isShipnstock, senderShipnstockStates]);

  useEffect(() => {
    if (isShipnstock && receiverShipnstockStates?.length) {
      const matchedState = receiverShipnstockStates.find(
        (state: any) =>
          state?.name?.toLowerCase() ==
          data?.consignee_state?.trim()?.toLowerCase(),
      );
      if (matchedState) {
  ...
  }, [isShipnstock, receiverShipnstockStates]);
  ...
  const handleDataFilter = (allData: any = [], is_ddp: number = 1) => {
    if (data?.shipment_type != "4" && data?.booking_type != "2") {
      const filterData = allData?.filter(
        (item: any) => item?.is_ddp == (is_ddp == 2 ? 1 : 0),
      );
      setVendorData(filterData || []);
    } else {
      setVendorData(allData || []);
    }
  };

  useEffect(() => {
    if (data?.shipment_type != "4" && data?.booking_type != "2") {
      handleDataFilter(allVendorData, data?.incoterm);
    }
  }, [data?.incoterm]);
  ...
  const checkAcl = async (booking_charges: any) => {
    setAclSpinner(true);
    try {
      const response = await Check_available_credit_limit(
        data?.franchisee_id,
        booking_charges,
        data?.airwaybill_no,
      );
  ...
  const [oldCourierId, setOldCourierId] = useState(
    awbData?.pickup_data?.courier_id || "",
  );
  ...
  const handleEdit = () => {
    if (
      ...
      if (data?.booking_type == "1" && isValidHsn(editDimensionData.hsn_code)) {
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8  p-2 rounded-lg ">
          ...
          <div>
            <FormLabel htmlFor="regular-form-1">
              Weight (in{" "}
              {`${data?.unit?.weight_unit ? data?.unit?.weight_unit : ""}`})
            </FormLabel>
            <FormInput
              type="number"
              placeholder="Weight"
          ...
          <div>
            <FormLabel htmlFor="regular-form-1">
              Value (in{" "}
              {`${awbData?.get_currency_data[0]?.currency
                ? awbData?.get_currency_data[0]?.currency
                : ""
                }`}
              )
  ...
  const handleStepTwo = async () => {
    ...
    try {
      const res = await Price_comparison(rateData);
      if (res?.data?.status == 200) {
        setAllVendorData(res?.data?.data);
        handleDataFilter(res?.data?.data || [], data?.incoterm || 0);
      } else {
        setAllVendorData([]);
      }
      setCurrentStep(2);
  ...
  const [senderDetails, setSenderDetails] = useState({
    ...
    ...(data?.courier_code.includes("aramex") || data?.courier_code.includes("sf_express")
      ? {
        ...
        ...(data?.courier_code.includes("aramex") ? { export_type: data?.export_type || "" } : {}),
        ...(data?.courier_code.includes("sf_express") ? { tax_id: data?.tax_id || "" } : {})
      }
      : {}),
      ...
      <div className="grid grid-cols-12 gap-6 gap-y-3 ">
        ...
        <div className="col-span-12 sm:col-span-6">
          ...
          <FormSelect
            ...
            disabled={!data?.courier_code.includes("dhl")}
            onChange={(e) =>
              setSenderDetails((prev) => ({
                ...prev,
          ...
          <FormLabel htmlFor="modal-form-5">
            {consignerDocTypes &&
              consignerDocTypes?.find(
                (elem) => elem.id == data?.consigner_doc_type,
              )?.value}{" "}
            <span className="text-red-500">*</span>
          </FormLabel>
        ...
        {data?.courier_code.includes("aramex") || data?.courier_code.includes("sf_express") ? (
          <>
            {data?.courier_code.includes("aramex") ? (<div className="col-span-12 sm:col-span-6">
              <FormLabel htmlFor="modal-form-6">
            ...
            {data?.courier_code.includes("sf_express") ? (<div className="col-span-12 sm:col-span-6">
  ...
  const [receiverDetails, setReceiverDetails] = useState({
    ...
    ...(data?.destination_country_code == "MX" ||
      ...
      ? {
        consignee_tax_id: data?.consignee_tax_id || "",
      }
      : {}),
    ...(data?.import_booking == 2
      ? {
  ...
  const changeVendor = async () => {
    ...
    try {
      ...
      const result = await Change_vendor_api({
        ...data,
        counter: skartCounter,
        ...(data?.courier_code.includes("widect")
          ? { manufacturer_id: MID }
  ...
  return (
    <>
      <div className="overflow-y-scroll h-[550px] md:h-full md:overflow-y-auto mb-8">
        <div
          className="p-2 my-2 cursor-pointer rounded-full shadow-lg mr-4 w-8 bg-white"
          onClick={() => {
            setAwbNo("");
            setShowChangeVendor(false);
          }}
        >
          <Lucide
            icon="ArrowLeft"
          ...
          <div className="flex flex-col-reverse md:flex-row gap-4 md:gap-0 justify-between w-full">
            ...
            >
              <div>
                ...
                <div className="flex gap-2 ">
                  ...
                  <div className="mx-auto p-1 h-14 min-w-28 border-2 rounded flex flex-col  justify-center">
              ...
              <div>
                ...
                <div className="flex gap-2 ">
                  ...
                  <div className="mx-auto p-1  min-w-28 h-14 border-2 rounded flex flex-col  justify-center text-wrap">
                    <h1 className="font-medium text-sm md:text-lg whitespace-nowrap overflow-hidden overflow-ellipsis">
                      {data?.destination_country}
                    </h1>
                    <p className="whitespace-nowrap overflow-hidden overflow-ellipsis">
            ...
            {franchiseeData && (
              <div className="intro-x rounded-lg w-full md:w-[35%] h-full">
                <div className="flex items-center px-4 py-3 box h-full w-full">
        ...
        <div className="mt-4 flex flex-col md:flex-row gap-4 md:gap-0  justify-between tbl-overflow-x-auto">
          <div className=" w-full md:w-[63%]">
            <Disclosure defaultOpen>
              {({ open }) => (
                <div className="bg-white rounded-lg intro-y w-full h-max">
                  <Disclosure.Button
                          ...
                          <div className="grid grid-cols-1  md:grid-cols-2 gap-4">
                            <div className="flex gap-4 w-full">
                              ...
                              >
                                <FormLabel
                                  htmlFor="directParty"
                                  className="block mb-2 text-base font-small text-gray-900 dark:text-white"
                                >
                                  SHIPMENT TYPE{" "}
                                  <span className="text-red-500">*</span>
                                </FormLabel>
                            ...
                            <div>
                              ...
                              <div className="flex gap-4">
                                <FormSelect className="uppercase" disabled>
                                  <option value={data?.unit?.weight_unit}>
            ...
            <div className="w-full md:hidden block mt-4">
              {currentFaq >= 2 ? (
                isVendorLoading ? (
                  <div className="w-full h-72 my-8 flex justify-center items-center">
                    <LoadingIcon
                      icon="tail-spin"
                  ...
                  <>
                    <Disclosure as="div" defaultOpen className="w-full ">
                      {({ open }) => (
                            ...
                            >
                                  ...
                                  <Table.Tbody className="p-0">
                                    {vendorData.length > 0 &&
                                      vendorData
                                        ?.sort(
                                          (a, b) =>
                                            Number(a?.grand_total_without_gst) -
                                            Number(b?.grand_total_without_gst),
                                        )
                                        ?.map((elem, index) => (
                                          <Table.Tr
                              ...
                              <div className="flex justify-end my-3">
                                <Button
                                  ...
                                  disabled={
                                    !selectVendor ||
                                    (data?.courier_id == oldCourierId &&
                                      Number(oldCounter) < 3) ||
                                    shipmentResponse ||
                                    aclSpinner
                                  }
                                  className="w-[35%] bg-mustard border-none text-white text-lg font-medium"
                                  onClick={async (
                                    e: React.MouseEvent<HTMLButtonElement>,
                                  ) => {
                                    if (data?.courier_code.includes("widect")) {
                                      if (!data?.incoterm) {
                                        if (isRestricted) {
                                          setCurrentFaq(1);
                                          setCurrentStep(1);
                                    ...
                                    if (data?.courier_code.includes("skynet")) {
                                      getSkynetCode();
                                    }
                                    ...
                                    if (
                                      ...
                                      if (data?.courier_code.includes("dhl")) {
                                        if (data?.shipment_type == "2") {
                                          if (
                                            (data as any)?.description?.length >
                                            90
                                          ) {
                                            showAlert(
            ...
            {currentFaq >= 3 && (
              <Disclosure as="div" className="mt-4 w-full" defaultOpen>
                {({ open }) => (
                  <div className="bg-white rounded-lg intro-y">
                    <Disclosure.Button
                      onClick={() => setCurrentStep(3)}
                      className="flex w-full justify-between rounded-lg bg-white px-4 py-2 text-left text-sm font-medium h-max focus:outline-none focus-visible:ring focus-visible:ring-purple-500/75"
                    >
                ...
                {({ open }) => (
                  <div className="bg-white rounded-lg intro-y">
                    <Disclosure.Button
                      ...
                      className="flex w-full justify-between rounded-lg px-4 py-2 text-left text-sm font-medium h-max focus:outline-none focus-visible:ring focus-visible:ring-purple-500/75"
                    >
          ...
          {currentFaq >= 2 ? (
            isVendorLoading ? (
              <div className="w-full md:w-[35%] h-72 my-8 hidden md:flex justify-center items-center">
                <LoadingIcon
                  icon="tail-spin"
                  className="block m-auto w-[35%] "
                />
              ...
              <div className="hidden md:flex justify-center w-full md:w-[35%]">
                <img src={ErrorGif} alt="error-gif" className="w-48 h-24" />
              </div>
            ) : allVendorData.length > 0 ? (
              <>
                              ...
                              <div className="py-3 flex justify-center">
                                <FormSwitch>
                                  ...
                                  <FormSwitch.Input
                                    ...
                                    disabled={
                                      shipmentResponse || aclSpinner || spinner
                                        ? true
                                        : false
                                    }
                                  />
                          ...
                          <div className="flex justify-end my-3">
                            <Button
                              ...
                              onClick={async (
                                ...
                                if (data?.courier_code.includes("emirates")) {
                                  setReceiverDetails((prev: any) => ({
                                    ...prev,
                                    is_residential:
                                      Number(
                                ...
                                if (
                                  ...
                                  if (data?.courier_code.includes("dhl")) {
                                    if (data?.shipment_type == "2") {
                                      if (
                                        (data as any)?.description?.length > 90
                                      ) {
                                        showAlert(
                                          "Description length should be less than 90 characters in Dhl",
                                          "warning",
